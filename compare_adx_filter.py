"""
ADX 最低开仓门槛回测
在 MA15/50 + SMA200 基础上，ADX < 阈值时不开仓
"""
import os, sys
import numpy as np
import pandas as pd
...
ADX_THRESHOLDS = [0, 15, 18, 20, 22, 25]
...
def run_one(df, sym, adx_min):
    ...
    for i in range(1, len(df)):
        ...
        # ADX 过滤：信号触发但 ADX 太低 → 跳过开仓（仍然平反向仓位）
    ...
    return eq_df, trade_count, filtered_count
...
def stats(eq, initial=10000):
    ...
    return {"ret": ret, "sharpe": sharpe, "dd": max_dd, "yearly": yearly}
...
def main():
    ...
    print("=" * 110)
    print("ADX 最低开仓门槛回测（MA15/50 + SMA200, 3x, BTC+LINK+BCH, 2021-2025）")
    print("=" * 110)
    print(f"\n{'ADX门槛':>8}{'Return':>10}{'Sharpe':>10}{'MaxDD':>10}{'收益/DD':>10}{'交易':>6}{'过滤':>6}{'2021':>8}{'2022':>8}{'2023':>8}{'2024':>8}{'2025':>8}")
    print("-" * 110)
    ...
    for thresh in ADX_THRESHOLDS:
        ...
        total_filtered = 0
        for sym in SYMBOLS:
            eq, trades, filtered = run_one(data[sym], sym, thresh)
        ...
        y = s["yearly"]
        results[thresh] = s
        label = "当前" if thresh == 0 else ""
        print(f"{thresh:>8}{s['ret']:>+10.1%}{s['sharpe']:>10.2f}{s['dd']:>10.1%}{rar:>10.2f}{total_trades:>6}{total_filtered:>6}{y.get(2021,0):>+8.1%}{y.get(2022,0):>+8.1%}{y.get(2023,0):>+8.1%}{y.get(2024,0):>+8.1%}{y.get(2025,0):>+8.1%} {label}")
    ...
    for thresh in ADX_THRESHOLDS:
        if thresh == 0:
            continue
        s = results[thresh]
        delta_ret = s["ret"] - base["ret"]
        delta_sharpe = s["sharpe"] - base["sharpe"]
        delta_dd = s["dd"] - base["dd"]
        print(f"  ADX≥{thresh}: Δ收益 {delta_ret:+.1%} | ΔSharpe {delta_sharpe:+.2f} | ΔDD {delta_dd:+.1%}")
